...
const DEFAULT_PROMPT =
  "You are a thoughtful writer who speaks in a natural, specific voice. Match the tone, cadence, and vocabulary of the provided samples.";

export function renderSource(container, state, onBuilt) {
  container.innerHTML = `
    <h2 class="step-heading">Source</h2>
    <p class="step-desc">Upload writing samples or chat exports, name the persona, then build a training dataset.</p>

    <div class="source-layout">
      <div class="section-card">
        <h3 class="section-card-title">1. Upload samples</h3>
        <div class="drop-zone" id="drop-zone" tabindex="0" role="button" aria-label="Upload files">
          ...
          <p class="drop-zone-hint">.txt · .jsonl · Discord / WhatsApp / Instagram exports</p>
        </div>
      ...
      <div class="section-card">
        <h3 class="section-card-title">2. Persona details</h3>
        <div class="form-grid">
          <div class="form-row">
            <label for="persona-name">Persona name</label>
            <input type="text" id="persona-name" placeholder="e.g. Alex" autocomplete="off" value="${escapeAttr(state.form.personaName)}" />
          </div>
          <div class="form-row">
            <label for="system-prompt">System prompt</label>
            <textarea id="system-prompt" placeholder="Describe how this persona should speak…">${escapeHtml(state.form.systemPrompt)}</textarea>
            <button type="button" class="btn btn-ghost btn-sm" id="use-default-prompt">Use starter prompt</button>
          </div>
          <div class="form-row" id="target-name-row" hidden>
            ...
            <p class="field-hint">Required for Discord, WhatsApp, and Instagram exports so we know whose messages become the assistant.</p>
          </div>
          <div class="checkbox-row">
            <input type="checkbox" id="synthetic-qa" ${state.form.syntheticQa ? "checked" : ""} />
            <label for="synthetic-qa">Generate synthetic Q&amp;A pairs</label>
          </div>
        </div>
        <div class="btn-row">
    ...
    <div id="build-success" class="callout callout-success" hidden></div>
  ...
  const successEl = container.querySelector("#build-success");
  const personaNameEl = container.querySelector("#persona-name");
  const systemPromptEl = container.querySelector("#system-prompt");
  ...
  container.querySelector("#use-default-prompt").addEventListener("click", () => {
    if (systemPromptEl.value.trim() && !window.confirm("Replace the current system prompt with the starter?")) {
      return;
    }
    systemPromptEl.value = DEFAULT_PROMPT;
    state.form.systemPrompt = DEFAULT_PROMPT;
    systemPromptEl.focus();
  });

  function updateUploadUI() {
    if (!state.uploads.length) {
      uploadList.innerHTML = '<li class="upload-empty">No files yet — drop samples above.</li>';
    } else {
      uploadList.innerHTML = state.uploads
        .map(
          (u, i) => `
        <li class="upload-item">
          <span class="upload-name" title="${escapeAttr(u.filename)}">${escapeHtml(u.filename)}</span>
    ...
    if (state.uploads.length > 0) {
      ...
      formatHint.textContent = last.sample_hint || `Detected as ${last.format_label || last.detected_format}.`;
      if (!state.form.personaName && last.filename) {
        const guess = last.filename.replace(/\.[^.]+$/, "").replace(/[_-]+/g, " ").trim();
        if (guess) {
          personaNameEl.placeholder = `e.g. ${guess}`;
        }
      }
    } else {
  ...
  buildBtn.addEventListener("click", async () => {
    const personaName = personaNameEl.value.trim();
    ...
    if (!personaName || !systemPrompt) {
      ...
      if (!personaName) personaNameEl.focus();
      else systemPromptEl.focus();
      return;
    }
    ...
    if (needsTarget && !targetName) {
      ...
      container.querySelector("#target-name").focus();
      return;
    }
    ...
    try {
      ...
      successEl.innerHTML = `
        <strong>Dataset ready — ${result.total_examples} examples</strong>
        Next: open Train to fine-tune, or keep adding samples and rebuild.
        <div class="btn-row mt-3">
          <button type="button" class="btn btn-primary btn-sm" id="goto-train">Continue to Train →</button>
        </div>
      `;
      successEl.querySelector("#goto-train")?.addEventListener("click", () => {
        if (typeof window.__pfGoToStep === "function") window.__pfGoToStep("train");
      });
